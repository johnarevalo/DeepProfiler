...
def make_dataset(path, batch_size, single_cell_metadata, labels, is_training):
    @tf.function
...
def setup_callbacks(config, strategy_lr):
    ...
    def cosine():
        ...
        return lr_schedule_lr, lr_schedule_epochs

    if "lr_schedule" in config["train"]["model"] and config["train"]["model"]["lr_schedule"] == "cosine":
        lr_schedule_lr, lr_schedule_epochs = cosine()
    ...
    return [callback_model_checkpoint, callback_csv, tf.keras.callbacks.LearningRateScheduler(lr_schedule, verbose=1)]
...
def learn_model(config, epoch):
    ...
    labels = tf.keras.utils.to_categorical(single_cell_metadata["Class_Name"])
    num_classes = len(pd.unique(single_cell_metadata["Class_Name"]))
    single_cell_metadata["Categorical"] = labels
